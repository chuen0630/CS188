...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for elimVar in eliminationOrder:
            notJoint, joint = joinFactorsByVariable(currentFactorsList, elimVar)
            if len(joint.unconditionedVariables()) == 1:
                currentFactorsList = notJoint
                continue
            elimJoint = eliminate(joint, elimVar)
            notJoint.append(elimJoint)
            currentFactorsList = notJoint
            counter += 1
        VE = normalize(joinFactors(currentFactorsList))
        return VE
