...
def constructBayesNet(gameState):
    ...
    for house_var in HOUSE_VARS:
        for obsVar in obsVars:
            edges.append((house_var, obsVar))
    #enum variableDomainsDict
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    for obsPos in gameState.getHouseWalls(bottomLeftPos):
        obsVar = OBS_VAR_TEMPLATE % obsPos
        obsFactor = bn.Factor([obsVar], HOUSE_VARS  , bayesNet.variableDomainsDict())
        for GHPos in HOUSE_VALS:
            for FHPos in HOUSE_VALS:
                red = {obsVar:RED_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                blue = {obsVar:BLUE_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                noObs = {obsVar:NO_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                if FHPos == BOTTOM_LEFT_VAL:
                    obsFactor.setProbability(red, PROB_FOOD_RED)
                    obsFactor.setProbability(blue, 1-PROB_FOOD_RED)
                    obsFactor.setProbability(noObs, 0.0)
                elif GHPos == BOTTOM_LEFT_VAL:
                    obsFactor.setProbability(red, PROB_GHOST_RED)
                    obsFactor.setProbability(blue, 1-PROB_GHOST_RED)
                    obsFactor.setProbability(noObs, 0.0)
                else:
                    obsFactor.setProbability(red, 0.0)
                    obsFactor.setProbability(blue, 0.0)
                    obsFactor.setProbability(noObs, 1.0)
        bayesNet.setCPT(obsVar, obsFactor)

    for obsPos in gameState.getHouseWalls(topLeftPos):
        obsVar = OBS_VAR_TEMPLATE % obsPos
        obsFactor = bn.Factor([obsVar], HOUSE_VARS  , bayesNet.variableDomainsDict())
        for GHPos in HOUSE_VALS:
            for FHPos in HOUSE_VALS:
                ...
                blue = {obsVar:BLUE_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                noObs = {obsVar:NO_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                if FHPos == TOP_LEFT_VAL:
                    obsFactor.setProbability(red, PROB_FOOD_RED)
                    obsFactor.setProbability(blue, 1-PROB_FOOD_RED)
                    obsFactor.setProbability(noObs, 0.0)
                elif GHPos == TOP_LEFT_VAL:
                    obsFactor.setProbability(red, PROB_GHOST_RED)
                    obsFactor.setProbability(blue, 1-PROB_GHOST_RED)
                    obsFactor.setProbability(noObs, 0.0)
                else:
    ...
    for obsPos in gameState.getHouseWalls(bottomRightPos):
        obsVar = OBS_VAR_TEMPLATE % obsPos
        obsFactor = bn.Factor([obsVar], HOUSE_VARS  , bayesNet.variableDomainsDict())
        for GHPos in HOUSE_VALS:
            for FHPos in HOUSE_VALS:
                ...
                blue = {obsVar:BLUE_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                noObs = {obsVar:NO_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                if FHPos == BOTTOM_RIGHT_VAL:
                    obsFactor.setProbability(red, PROB_FOOD_RED)
                    obsFactor.setProbability(blue, 1-PROB_FOOD_RED)
                    obsFactor.setProbability(noObs, 0.0)
                elif GHPos == BOTTOM_RIGHT_VAL:
                    obsFactor.setProbability(red, PROB_GHOST_RED)
                    obsFactor.setProbability(blue, 1-PROB_GHOST_RED)
                    obsFactor.setProbability(noObs, 0.0)
                else:
    ...
    for obsPos in gameState.getHouseWalls(topRightPos):
        obsVar = OBS_VAR_TEMPLATE % obsPos
        obsFactor = bn.Factor([obsVar], HOUSE_VARS  , bayesNet.variableDomainsDict())
        for GHPos in HOUSE_VALS:
            for FHPos in HOUSE_VALS:
                ...
                blue = {obsVar:BLUE_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                noObs = {obsVar:NO_OBS_VAL, GHOST_HOUSE_VAR: GHPos, FOOD_HOUSE_VAR: FHPos}
                if FHPos == TOP_RIGHT_VAL:
                    obsFactor.setProbability(red, PROB_FOOD_RED)
                    obsFactor.setProbability(blue, 1-PROB_FOOD_RED)
                    obsFactor.setProbability(noObs, 0.0)
                elif GHPos == TOP_RIGHT_VAL:
                    obsFactor.setProbability(red, PROB_GHOST_RED)
                    obsFactor.setProbability(blue, 1-PROB_GHOST_RED)
                    obsFactor.setProbability(noObs, 0.0)
                else:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    VE = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    highestProb = 0
    prediction = None
    for assignment in VE.getAllPossibleAssignmentDicts():
        prob = VE.getProbability(assignment)
        if prob > highestProb:
            highestProb = prob
            prediction = assignment[FOOD_HOUSE_VAR]
    return {FOOD_HOUSE_VAR: prediction}
...
class VPIAgent(BayesAgent):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in query.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftFoodProb = query.getProbability(assignment)
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                rightFoodProb = query.getProbability(assignment)
        leftExpectedValue = leftFoodProb * WON_GAME_REWARD + rightFoodProb * GHOST_COLLISION_REWARD
        rightExpectedValue = leftFoodProb * GHOST_COLLISION_REWARD + rightFoodProb * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        explorProbNOut = self.getExplorationProbsAndOutcomes(evidence)
        for prob, outcome in explorProbNOut:
            newEvid = evidence.copy()
            newEvid.update(outcome)
            maxValueFromNewEvid = max(self.computeEnterValues(newEvid, enterEliminationOrder))
            expectedValue += prob*maxValueFromNewEvid
